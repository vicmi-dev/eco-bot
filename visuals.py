...
class VisualsScreen:
    ...
    def buttons(self):
        ...
        quit_button = Button(root, text="Quit",  command=root.destroy, bg= "black",fg = "white", font=('ariel', 11, 'bold'),)       
        # placing the Quit button on the screen
        quit_button.place(x=850,y=50)

    def labels(self):

        description = tk.Label(canvas1, text='The Eco-Bot is a ML&CS FG Python Script that help us clean our folders from files we do not use anymore', bg = "white", font = ('ariel', 10,'bold'))
        description.place(x = 10, y = 50)

        """ LABEL FOR FILE SIZE """
        #Label for the question
        label2 = tk.Label(canvas1, text='What do you consider as too many bytes?', bg = "white")
        ...
        entry1.place(x = 10, y = 120)#canvas1.create_window(400, 100, window=entry1)

        """ LABEL FOR KEYWORD """
        #Label for the question
        label2 = tk.Label(canvas1, text='Keyword of files you want to resize/delete', bg = "white")
        label2.config(font=('ariel', 10))
        ...
        entry2.place(x = 10, y =190)#canvas1.create_window(400, 150, window=entry2)

        """ LABEL FOR DATE PICKER"""
        #Label for the question
        label2 = tk.Label(canvas1, text='Select the date you want to check files previous from', bg = "white")
        label2.config(font=('ariel', 10))
        ...
        style.configure('my.DateEntry',
                ...
                arrowcolor='white')
        dateentry = DateEntry(canvas1,style='my.DateEntry')
        dateentry.grid(row=0,column=0,padx=10, pady = 250)
        #dateentry.pack()
...
canvas1 = tk.Canvas(root, width = 900, height = 565)
#Display Image
